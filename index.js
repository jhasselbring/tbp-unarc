...
// --- Configuration ---
const EXTRACTION_TIMEOUT_MS = 1 * 60 * 1000; // 30 minutes timeout per file
...
const shouldDelete = args.includes('-d'); // <-- Check for the -d flag
...
if (shouldDelete) {
    console.log(`${colors.yellow}Delete flag (-d) detected. Original archives will be deleted after successful extraction.${colors.reset}`);
}
    ...
    for (const archivePath of archives) { // Use the full path now
      ...
      switch (fileExt) {
        case '.zip':
          ...
          break;
        case '.rar':
          // Add -p- to prevent password prompts causing hangs
          command = `unrar x -o+ -p- ${quotedArchivePath} ${quotedOutputDir}${path.sep}`;
          break;
        case '.7z':
      ...
      try {
        ...
        console.log(`Extracting ${archiveFile} to ${outputDir}... (Timeout: ${EXTRACTION_TIMEOUT_MS / 60000} mins)`);

        // Execute with timeout
        const { stdout, stderr } = await execPromise(command, { timeout: EXTRACTION_TIMEOUT_MS });
        ...
        extractionSuccessful = true; // Mark as successful
          ...
          if (error.signal === 'SIGTERM' && error.killed) { // Check if killed due to timeout
                 console.error(`${colors.red}  -> Error: Extraction timed out after ${EXTRACTION_TIMEOUT_MS / 60000} minutes.${colors.reset}`);
                 console.error(`${colors.red}     (Possible causes: very large file, password prompt hang, or other issue).${colors.reset}`);
          } else if (error.code === 'ENOENT' || error.message.includes('command not found') || (process.platform === 'win32' && error.message.includes('is not recognized'))) {
               ...
               if (error.stderr) {
                 ...
                 // Add specific check for password error messages in stderr if available
                if (error.stderr && (error.stderr.toLowerCase().includes('password required') || error.stderr.toLowerCase().includes('enter password'))) {
                     console.error(`${colors.red}  -> Possible password required for archive.${colors.reset}`);
                }
          }
      ...
      // 7. Delete archive if extraction was successful AND -d flag is set
      if (extractionSuccessful && shouldDelete) { // <--- Added check for shouldDelete
          try {
              console.log(`Attempting to delete original archive (due to -d flag): ${archivePath}...`);
      ...
      } else if (extractionSuccessful && !shouldDelete) {
            // Optional: Log that deletion is skipped if successful but no -d flag
            console.log(`Extraction successful, skipping deletion of ${archiveFile} (no -d flag).`);
        }
    } // End of for...of loop
